...
@implementation LocalNotificationsUtil
...
+ (FIRApp *)getApp:(NSString *)appDisplayName {
  NSString *appName = [LocalNotificationsUtil getAppName:appDisplayName];
...
+ (void)sendJSEventWithAppName:(RCTEventEmitter *)emitter app:(FIRApp *)app name:(NSString *)name body:(id)body {
  ...
  newBody[@"appName"] = [LocalNotificationsUtil getAppDisplayName:app.name];

  [LocalNotificationsUtil sendJSEvent:emitter name:name body:newBody];
}
