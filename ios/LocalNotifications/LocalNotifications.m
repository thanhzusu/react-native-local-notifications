...
#import "LocalNotificationsEvents.h"
#import "LocalNotificationsUtil.h"
...
@interface LocalNotifications () <UNUserNotificationCenterDelegate>
#else
...
static LocalNotifications *theRNTeachNotifications = nil;
...
+ (nonnull instancetype)instance {
    return theRNTeachNotifications;
}

+ (void)configure {
    // PRE-BRIDGE-EVENTS: Consider enabling this to allow events built up before the bridge is built to be sent to the JS side
    // pendingEvents = [[NSMutableArray alloc] init];
    theRNTeachNotifications = [[LocalNotifications alloc] init];
}
...
- (id)init {
    self = [super init];
    if (self != nil) {
        NSLog(@"Setting up LocalNotifications instance");
        [self initialise];
    }
    return self;
}
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
    ...
    if (isFcm || isScheduled) {
        if (isScheduled) {
            ...
            event = LOCAL_NOTIFICATIONS_DISPLAYED;
        } else {
            // don't show notification
            options = UNNotificationPresentationOptionNone;
            // notification_received
            event = LOCAL_NOTIFICATIONS_RECEIVED;
        }
    } else {
...
didReceiveNotificationResponse:(UNNotificationResponse *)response
     ...
     [self sendJSEvent:self name:LOCAL_NOTIFICATIONS_OPENED body:message];
     if (handlerKey != nil) {
...
// we create a temporary instance of LocalNotifications.
...
- (void)sendJSEvent:(RCTEventEmitter *)emitter name:(NSString *)name body:(id)body {
    if (emitter.bridge && jsReady) {
        ...
        } else if ([name isEqualToString:LOCAL_NOTIFICATIONS_OPENED]) {
...
- (NSArray<NSString *> *)supportedEvents {
    return @[LOCAL_NOTIFICATIONS_DISPLAYED, LOCAL_NOTIFICATIONS_OPENED, LOCAL_NOTIFICATIONS_RECEIVED];
}

- (NSDictionary *) constantsToExport {
    NSMutableDictionary *constants = [NSMutableDictionary new];
    constants[@"backgroundFetchResultNoData"] = @([RCTConvert NSInteger:@(UIBackgroundFetchResultNoData)]);
    constants[@"backgroundFetchResultNewData"] = @([RCTConvert NSInteger:@(UIBackgroundFetchResultNewData)]);
    constants[@"backgroundFetchResultFailed"] = @([RCTConvert NSInteger:@(UIBackgroundFetchResultFailed)]);
    return constants;
}
