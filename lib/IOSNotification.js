...
export default class IOSNotification {
  constructor(notification, notifications, data) {
    ...
    const complete = fetchResult => {
      const { notificationId } = notification; // && notifications check for Flow

      if (notificationId && notifications) {
        console.log(
          `###Completion handler called for notificationId=${notificationId} and fetchResult=${fetchResult}`,
        );
        notifications.nativeModule.complete(notificationId);
      }
    };

    if (notifications.ios.shouldAutoComplete) {
      console.log(
        `###shouldAutoComplete: ${notifications.ios.shouldAutoComplete}`,
      );
