...
/**
 *
 * LocalNotification representation wrapper
 */
...
const NativeEventManager = Platform.select({
  ios: new NativeEventEmitter(NativeModules.LocalNotification),
...
class LocalNotification {
  constructor() {
    this.nativeModule = NativeModules.LocalNotification;
  ...
  cancelNotification(notificationId) {
    if (!notificationId) {
      return Promise.reject(
        new Error(
          'LocalNotification: cancelNotification expects a `notificationId`',
        ),
      );
    }
  ...
  displayNotification(notification) {
    if (!(notification instanceof Notification)) {
      return Promise.reject(
        new Error(
          `LocalNotification:displayNotification expects a 'Notification' but got type ${typeof notification}`,
        ),
      );
    }
  ...
  onNotification(nextOrObserver) {
    ...
    if (_.isFunction(nextOrObserver)) {
      ...
      throw new Error(
        'LocalNotification.onNotification failed: First argument must be a function or observer object with a `next` function.',
      );
    }
  ...
  onNotificationDisplayed(nextOrObserver) {
    ...
    if (_.isFunction(nextOrObserver)) {
      ...
      throw new Error(
        'LocalNotification.onNotificationDisplayed failed: First argument must be a function or observer object with a `next` function.',
      );
    }
  ...
  onNotificationOpened(nextOrObserver) {
    ...
    if (_.isFunction(nextOrObserver)) {
      ...
      throw new Error(
        'LocalNotification.onNotificationOpened failed: First argument must be a function or observer object with a `next` function.',
      );
    }
  ...
  removeDeliveredNotification(notificationId) {
    if (!notificationId) {
      return Promise.reject(
        new Error(
          'LocalNotification: removeDeliveredNotification expects a `notificationId`',
        ),
      );
    }
  ...
  scheduleNotification(notification, schedule) {
    if (!(notification instanceof Notification)) {
      return Promise.reject(
        new Error(
          `LocalNotification:scheduleNotification expects a 'Notification' but got type ${typeof notification}`,
        ),
      );
    }
